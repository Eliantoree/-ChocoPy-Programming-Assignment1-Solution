...
**Download Link:https://programming.engineering/product/chocopy-programming-assignment1-solution/**
...
kind FuncDef extends Declaration { name: Identifier,
...
kind VarDef extends Declaration { var: TypedVar,
...
kind TypeAnnotation extends Node { }

kind ClassType

extends TypeAnnotation { className: string
...
extends TypeAnnotation { elementType: TypeAnnotation

}

kind Stmt extends Node { }

kind ExprStmt extends Stmt { expr: Expr

}

kind ReturnStmt extends Stmt { value: Expr?

}

kind AssignStmt extends Stmt { targets: [Expr],
...
kind WhileStmt extends Stmt { condition: Expr,

body: [Stmt]

}

kind ForStmt extends Stmt { identifier: Identifier, iterable: Expr,
...
kind Identifier extends Expr { name: string

}

kind BinaryExpr extends Expr { left: Expr,
...
kind UnaryExpr extends Expr { operator: string, operand: Expr
...
kind CallExpr extends Expr { function: Identifier, args: [Expr]

}

kind MethodCallExpr extends Expr { method: MemberExpr,

args: [Expr]

}

kind IndexExpr extends Expr { list: Expr,

index: Expr

}

kind MemberExpr extends Expr { object: Expr,

member: Identifier

}

kind ListExpr extends Expr { elements: [Expr]
...
kind StringLiteral extends Literal { value: string

}

kind IntegerLiteral extends Literal { value: int

}

kind BooleanLiteral extends Literal { value: bool

}

kind Errors extends Node { errors: [CompilerError]
...
1. What strategy did you use to emit INDENT and DEDENT tokens correctly?

2. What was the hardest language feature (not including indentation) to implement in this assign-ment? Why was it challenging?

3. What kind of parse errors does your parser detect? In what situations can it recover? Explain with the help of examples from the custom tests that you have written.
